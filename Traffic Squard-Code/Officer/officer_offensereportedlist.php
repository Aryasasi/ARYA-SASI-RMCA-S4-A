...
$sql="SELECT * FROM `offense committed` WHERE `district`='$asa' and `status`='pending' order by O_R_id desc";
if(isset($_POST['ofser'])){
  $ser = "%".$_POST['of_search']."%";
  $serc = $_POST['of_search'];
  $sql="SELECT * FROM `offense committed` WHERE `district`='$asa' and `status`='pending' and Vehicle_Number like '$ser' order by O_R_id desc";
}
if(isset($_POST['offilt'])){
  ...
  $toi = $_POST['tofil'];
  $tod = $_POST['tofil']."%";
  $sql="SELECT * FROM `offense committed` WHERE `district`='$asa' and `status`='pending' and reported_date between '$fod' and '$tod' order by O_R_id desc";
}
  }
...
<form method="post" action="officer_offensereportedlist.php">
...
while ($row = mysqli_fetch_array($result)) {
  ...
  $sql3=mysqli_query($con,"SELECT * FROM `tbl_role` where `role_id`='$role_id'");
    ...
    $count++;
			...
			<a href="officer_offensereportdelete.php?id=<?php echo $row['O_R_id']; ?>" class="delbutton" title="Click to Delete the Offense">
            <i class="fa fa-trash fa-lg text-danger"></i></a>&nbsp;
            <a href="mark_as_approved.php?uid=<?php echo $row['O_R_id'];?>" onclick="return confirm('Mark as Approved??')"><img src="images/600px-Approve_icon.svg.png" width="30px"></a>
													</td>
